...
read VERSION
./setup.py test  &&  ./setup.py register sdist upload  &&  git tag -a v$VERSION -m "Version $VERSION"  &&  git push --tags  &&  echo 'Now remember to change the version number to a "dev" suffix and commit.'
